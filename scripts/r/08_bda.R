...
native_ppred_response_00 <- brm(
  is_correct ~ 1 + 
    (1 | participant ) + (1 | speaker_variety/sentence_type/sentence) + 
    (1 | sentence_type), 
  data = natives, 
  prior = nat_priors, 
  sample_prior = "only",
...
formula <- bf(
  rt_adj ~ 0 + Intercept + 
    (1 | participant) + (1 | speaker_variety/sentence_type/sentence) + 
    (1 | sentence_type), 
  ndt ~ 0 + Intercept + (1 | participant)
  )

rt_slog_priors <- c(
  set_prior('normal(-0.69, 0.5)', class = 'Intercept'), # around  0.500 secs
  set_prior("normal(0, 0.5)",     class = "Intercept", dpar = "ndt"),
  set_prior('normal(0.4, 0.3)',   class = 'sigma'),        # SD of individual rts in log-units
  set_prior('normal(0.3, 0.1)',   class = 'sd')            # variability between participants
)

native_rt_01 <- brm(
  formula = formula, 
  data = natives %>% filter(rt_adj > 0 & rt_adj <= 5), 
  #prior = rt_slog_priors, 
  warmup = 1000, iter = 2000, chains = 6, 
  family = shifted_lognormal(), 
  cores = 4, 
