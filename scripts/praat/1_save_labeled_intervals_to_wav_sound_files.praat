...
form Save intervals to small WAV sound files
	...
	boolean Exclude_intervals_labeled_as_silent 1
	...
	sentence Folder ../../exp/stim/wavs/
	comment Give an optional prefix for all filenames:
	sentence Prefix andaluz_
...
for interval from start_from to end_at
	silent$ = Get label of interval... tier interval
	check = 0
	if silent$ = "silent" and exclude_intervals_labeled_as_silent = 1
		check = 1
	endif
	if silent$ = "" and exclude_empty_labels = 1
		check = 1
	endif
	if left$ (silent$,1) = "." and exclude_intervals_starting_with_dot = 1
		check = 1
	endif
	if check = 0
	   files = files + 1
...
for interval from start_from to end_at
	...
	check = 0
	if intname$ = "silent" and exclude_intervals_labeled_as_silent = 1
		check = 1
	endif
