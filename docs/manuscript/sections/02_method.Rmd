...
A total of `r n_learners` individuals completed a two-alternative forced choice (2AFC) task in which auditory stimuli were identified as being questions or statements. 
Participants were recruited using the Prolific.ac online experimental platform and were be compensated at a rate of $9.52 per hour for their time. 
...
The mean time to completion was `r round(t_mean, 2)` minutes (SD: `r round(t_sd, 2)`). 
The pool of participants was filtered using criteria set in Prolific.ac to insure participants self-reported as being L1 English speakers born, raised, and currently living in the US with no knowledge of any languages other than English and/or Spanish.
...
The study consisted of three tasks: a two-alternative forced choice task (2AFC), a lexical decision vocabulary assessment, and a lykert-type questionnaire to assess empathy. 
...
The auditory stimuli consisted of 64 critical items, 16 of each sentence type. 
To generate the stimuli, we recorded native Spanish speakers of eight different varieties (Cuban, standard Peninsular, Andalusian, Puerto Rican, Chilean, Argentinean, Mexican, and Peruvian). 
...
Prior to pre-registering our research questions and hypotheses we piloted the 2AFC experiment on 100 native Spanish speakers to assure the quality of the items and assess the difficulty of the task. 
...
The LexTALE is a lexical decision experiment used provide a standardized assessment of proficiency/vocabulary size in Spanish. 
In this task participants are presented words on the computer screen and must decide if it is real or not using the keyboard ('1' for real, '0' for fake). 
LexTALE scores can range from -20 to 60. 
Monolingual Spanish speakers generally score above 50. 
Scores from individuals with little or no knowledge of Spanish typically fall in the range of -20 to 0. 
We conceive of proficiency as a continuous variable, thus we consider a monolingual English speaker to have little to no proficiency in Spanish. 
Participant scores ranged from `r lt_dat$min_lt$val` to `r lt_dat$max_lt$val`, suggesting all proficiency levels were likely represented in the sample. 
The mean score was `r lt_dat$med_lt$val` (SD: `r lt_dat$sd_lt$val`). 
...
We planed to collect data from approximately 300 individuals: 100 monolingual Spanish speakers not reported here and 200 L2 learners). 
...
The second analysis utilized Bayesian drift diffusion modeling (DDM, RATCLIFF & MOON 2008). 
This approach to analyzing behavioral data models decision-making using a random-walk decision process.
...
We estimate the parameters off the DDM using a Bayesian methods and subsequently do regression on the posterior samples of the resulting parameters. 
...
A bias positive value indicates a preference for the upper boundary, whereas a negative bias is an indicator of a preference for the lower boundary. 
...
Conversely a lower drift rate is an indication of a more difficult task. 
...
We sample the posterior distribution of the model for statistical inferences. 
...
For statistical inferences, a posterior distribution for a parameter &beta; in which 95% of the HDI falls outside the ROPE and a high MPE (i.e., values close to 1) were taken as compelling evidence for a given effect. 
All exploratory analyses, explicitly described as such, include posterior point estimates, the 95% HDI and the MPE. 
We conducted the analyses using R and fit all models using the probabilistic programming language `stan` via the R package `brms` [@R_brms_a; @R_brms_b].
